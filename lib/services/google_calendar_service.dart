...
class GoogleCalendarService {
  ...
  static Future<bool> initialize() async {
    try {
      ...
      debugPrint(
          '🔑 Access token available: ${authentication.accessToken != null}');
      debugPrint('🔑 ID token available: ${authentication.idToken != null}');
      ...
      while (retryCount < maxRetries && httpClient == null) {
        try {
          debugPrint(
              '🔄 Attempt ${retryCount + 1} to get authenticated client...');

          httpClient = await _googleSignIn!.authenticatedClient().timeout(
            const Duration(seconds: 30), // Increased timeout
            onTimeout: () {
              debugPrint(
                  '⏰ Authenticated client request timed out after 30 seconds');
              return null;
            },
          );

          if (httpClient != null) {
            debugPrint('✅ Successfully got authenticated HTTP client');
            break;
          }
        } catch (e) {
          debugPrint(
              '❌ Error getting authenticated client (attempt ${retryCount + 1}): $e');
        }
      ...
      if (httpClient == null) {
        debugPrint(
            '❌ Failed to get authenticated HTTP client after $maxRetries attempts');
        debugPrint('🔧 Trying alternative authentication method...');
        ...
        try {
          ...
          debugPrint('❌ Alternative authentication failed: $e');
        }
  ...
  static Future<bool> forceReAuthentication() async {
    try {
      debugPrint('🔄 Forcing Google Calendar re-authentication...');
      ...
      try {
        ...
        debugPrint('⚠️ Sign out failed (continuing): $e');
      }

      // Force fresh sign-in
      debugPrint('🔐 Requesting fresh Google sign-in...');
      final account = await _googleSignIn!.signIn();

      if (account == null) {
        debugPrint('❌ User cancelled re-authentication');
        return false;
      }

      debugPrint('✅ Re-authentication successful: ${account.email}');

      // Now initialize normally
      return await initialize();
    } catch (e) {
      debugPrint('❌ Force re-authentication failed: $e');
      return false;
    }
  }
